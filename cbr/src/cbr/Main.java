...
public class Main {
  ...
  public static double getCrossValidatedAccuracy(List<DataInstance> dataInstances, Map<String, Attribute> attributes,
    ...
    double counter = 0.0;
    for (DataInstance data : dataInstances){
      Value val1 = data.getData().get(targetAttribute);
      DataInstance newData = getCopyWithRemovedAttribute(data,targetAttribute);
      List<DataInstance> newList = new ArrayList<>(dataInstances);
      newList.remove(data);
      Value val2 = classify(newList,attributes,newData,targetAttribute,distanceFunction);
    ...
    return (counter-1)/dataInstances.size();
  }
