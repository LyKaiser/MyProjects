...
public class QRCode {
    ...
    public static QRCode createValidatedFromBooleans(boolean[][] matrix) throws InvalidQRCodeException {
        ...
        if (versio > 6) {
            ...
            for (int i = 0; i < vInfoOben.length; i++) {
                if (vInfoOben[i]) {
                    validOben |= (1 << i);
                }
            }
            ...
            if (VersionInformation.fromBits(validOben) != null) {
                if (VersionInformation.fromBits(validUnten) == null &&
                        VersionInformation.fromBits(validOben).number() != versio) {
                    throw new InvalidQRCodeException();
                }
            }
            if (VersionInformation.fromBits(validUnten) != null) {
