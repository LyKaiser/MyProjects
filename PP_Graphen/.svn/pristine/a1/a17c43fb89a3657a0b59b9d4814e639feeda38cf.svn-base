...
public class SearchResultImpl<N, A> implements ObservableSearchResult<N, A> {
    ...
    Map<N, NodeStatus> statusMap=new HashMap<>();
    Map<N, NodeInformation<N, A>> infoMap=new HashMap<>();
    List<BiConsumer<N, SearchResult<N, A>>> onOpenLi=new ArrayList<>();
    List<BiConsumer<N, SearchResult<N, A>>> onCloseLi=new ArrayList<>();
    ...
    public Optional<Edge<N, A>> getPredecessor(N node) {
        ...
        if (info==null||info.getPredecssor()==null){
