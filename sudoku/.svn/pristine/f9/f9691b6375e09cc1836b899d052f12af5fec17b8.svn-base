...
class Main {
    ...
    public static Board forwardCheck(Board b, String name, Set<Constraint> cs) {
        ...
        for (String string : other) {
            if (isAssigned(b, string)) {
                b = forwardCheck(b, string, cs);
            }
        }
        ...
        if (!isSolved(b)) {
            if (findBestVar(b).isPresent()) {
                ...
                for (int i : b.get(best)) {
                    Board bNew = b;
                    bNew = bNew.set(best, i);
                    bNew = forwardCheck(bNew, best, cs);
                    if (solve(bNew, cs).isPresent()) {
                        return solve(bNew, cs);
                    }
                }
            } else return Optional.empty();
